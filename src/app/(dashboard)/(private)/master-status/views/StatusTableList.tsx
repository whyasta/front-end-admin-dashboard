...
import { deleteData, getData } from "@/app/server/masterStatusActions";
import OpenDialogOnElementClick from '@/components/dialogs/OpenDialogOnElementClick';
import DialogAddEditStatus from '../components/DialogAddEditStatus';
import ConfirmationDialog from '@/components/dialogs/confirmation-dialog';
import type { StatusType } from '@/types/pages/statusType';

type StatusTypeWithAction = StatusType & {
  action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<StatusTypeWithAction>()

const RcListTable = ({ tableData }: { tableData?: StatusType[] }) => {
    ...
    columnHelper.accessor('action', {
      ...
      cell: ({ row }) => (
        <div className='flex items-center'>
          ...
          }} dialog={DialogAddEditStatus} dialogProps={{ data: row.original, setRefresh: setRefresh }} />
  ...
  const tableColumns = useMemo<ColumnDef<StatusTypeWithAction, any>[]>(
    () =>
      isLoading
        ? columns.map((column) => ({
          ...column,
      ...
      <CardContent className='flex flex-col justify-between sm:flex-row max-sm:is-full items-start sm:items-center gap-4'>
        ...
        <div className='flex flex-col sm:flex-row max-sm:is-full items-start sm:items-center gap-4'>
          ...
          <OpenDialogOnElementClick element={Button} elementProps={{
            ...
            'className': 'max-sm:is-full',
            'children': 'Add'
          }} dialog={DialogAddEditStatus} dialogProps={{ setRefresh: setRefresh }} />
          <Button
            variant='contained'
